...
| 3 | Dashboard Web funcional | Vista web cargada correctamente | Abrir http://localhost:8080 | | 10 |
...
| 5 | Resolucion DNS entre contenedores | Dashboard consulta por nombre api-server | docker compose exec web-server python -c "import requests; print(requests.get('http://api-server:5000/api/system', timeout=5).status_code)" | | 15 |
...
| 8 | Persistencia historica en BD | Tabla metrics_history con registros | docker compose exec api-server python -c "import sqlite3; c=sqlite3.connect('/app/data/metrics.db'); print(c.execute('select count(*) from metrics_history').fetchone()[0])" | | 15 |
